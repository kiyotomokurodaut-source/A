...
(() => {
  ...
  if (!field) return;
  const topics = {english:'英語の読み方チェック6問',math:'数学の解説・方針の選び方',todai:'東大理系数学2026 第3問の解説','todai-strategy':'東大対策・東大受験戦略',juku:'塾選び・個別指導塾',plan:'学習計画・学習管理',mock:'模試の復習シート',homework:'中学受験の宿題',pricing:'料金・プラン',cases:'指導事例'};
  ...
  const sources={google:'Google検索',x:'X（Twitter）',twitter:'X（Twitter）',youtube:'YouTube',referral:'知人・紹介',material:'無料教材',site:'その他'};
  const qs=new URLSearchParams(location.search);
  const sourceKey=(qs.get('src')||qs.get('source')||qs.get('utm_source')||'').toLowerCase();
  if(sourceField&&sourceKey&&Object.hasOwn(sources,sourceKey))sourceField.value=sources[sourceKey];
})();
