...
const fields=[['consult-stage','学年・受験までの時期'],['consult-goal','目標・相談したい方向性'],['consult-current','今の学習環境'],['consult-trouble','一番困っている場面'],['consult-support','検討している支援'],['consult-source','黒田塾を知ったきっかけ'],['consult-topic','相談のきっかけ']];
...
byId('consult-build').addEventListener('click',()=>{const parts=fields.map(([id,label])=>[label,byId(id).value.trim()]).filter(([,v])=>v);if(!parts.length){say('まだ入力がありません。困っていることを一つ記入してください。');return;}output.value='黒田塾の無料受験戦略診断を希望しています。\n\n'+parts.map(([label,value])=>'【'+label+'】\n'+value).join('\n\n')+'\n\n現在の状況に合う支援について相談したいです。';say('相談文を作成しました。確認して編集できます。外部への送信はしていません。');});
